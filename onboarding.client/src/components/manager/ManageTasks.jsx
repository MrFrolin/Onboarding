...
function ManageTasks({ taskCategory }) {
    ...
    const publishData = async () => {
        ...
        if (selectedOption.length != 0) {
            await setTasksToSelectedMember(newTasks);
        } else {
    ...
    const setTasksToSelectedMember = async (newTasks) => {
        selectedOption.map(async (member, i) => {
            await axios.post(
                `https://localhost:7170/task/${member.value}`,
                addedTaskList,
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
    ...
    const addTask = async (newTaskName, newDescription) => {
        ...
        } else if (taskCategory == "Team-Tasks") {
            category = "Team-Tasks";
        }
    ...
    const updateCheckStatus = async (index) => {
        ...
        await axios.put(
            `https://localhost:7170/task/${firestoreUserObj.email}/${isCompleted}?taskName=${taskName}`
        );
    };
    ...
    const UpdateVideoURL = async (e) => {
        e.preventDefault();
        await axios.put(
            `https://localhost:7170/team/video/${firestoreUserObj.teamId}?videoUrl=${videoURL}`,
            {},
            {
                headers: {
                    Authorization: `Bearer ${token}`,
    ...
    const UpdateLink = async (e) => {
        e.preventDefault();
        await axios.put(
            `https://localhost:7170/team/link/${firestoreUserObj.teamId}?link=${newLink}`,
            {},
            {
                headers: {
                    Authorization: `Bearer ${token}`,
    ...
    const teamFilter = async (selectedOption) => {
    ...
    const getTeamMembers = async () => {
        await fetch(`https://localhost:7170/team/${firestoreUserObj.teamId}`, {
            method: "GET",
            headers: {
                Authorization: `Bearer ${token}`,
                "Content-Type": "application/json",
                        ...
                        {loading ? (
                            ...
                            <>
                                ...
                                <form className="max-w-xl mx-auto mt-5" onSubmit={handleSubmit}>
                                    ...
                                    >
                                        Use this to assign tasks to specific employees. Leave empty
                                        for all.
                                    </label>
                                        <Select
                                            options={teamMembers.map(
                                                ({ firestoreId, displayName, personalTaskNames}) => ({
                                                    value: firestoreId,
                                                    label: displayName,
                                                    personalTaskNames: personalTaskNames,
                                                })
                                            )}
                                        ...
                                        <ul>
                                            <Fade triggerOnce cascade damping={0.1}>
                                                ...
                                                {selectedOption.length === 1 ? (
                                                    <PersonalTasksList firestoreId={selectedOption[0].value} />
                                                ) : selectedOption.length === 0 ? (
                                                     <AssignmentList teamMembers={teamMembers} />
                                                ) : (
                                                    <AssignmentList teamMembers={selectedOption} />
                                                )}
                                        </ul>
                                    </div>
                                </div>
